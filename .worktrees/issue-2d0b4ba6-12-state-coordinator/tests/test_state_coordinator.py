...
class TestBoundaryValues:
    ...
    def test_empty_string_key(self):
        """Empty string as key is valid (edge case)."""
        coordinator = StateCoordinator()
        coordinator.set("conv", "", "value", "agent_1", {})
        assert coordinator.get("conv", "") == "value"

    def test_empty_string_value(self):
        """Empty string as value is valid."""
        coordinator = StateCoordinator()
        coordinator.set("conv", "key", "", "agent_1", {})
        assert coordinator.get("conv", "key") == ""

    def test_empty_conversation_id(self):
        """Empty conversation ID is valid (though unusual)."""
        coordinator = StateCoordinator()
        coordinator.set("", "key", "value", "agent_1", {})
        assert coordinator.get("", "key") == "value"

    def test_vector_clock_with_large_values(self):
        """Vector clock can handle large logical timestamps."""
        coordinator = StateCoordinator()
        vc = {"agent_1": 1000000}

        coordinator.set("conv", "key", "value", "agent_1", vc)
        assert vc["agent_1"] == 1000001

        retrieved_vc = coordinator.get_vector_clock("conv")
        assert retrieved_vc["agent_1"] == 1000001

    def test_vector_clock_with_many_agents(self):
        """Vector clock can track many agents (10+)."""
        coordinator = StateCoordinator()
        vc = {f"agent_{i}": 0 for i in range(20)}

        for i in range(20):
            agent = f"agent_{i}"
            coordinator.set("conv", f"key_{i}", f"val_{i}", agent, vc)

        retrieved_vc = coordinator.get_vector_clock("conv")
        assert len(retrieved_vc) == 20
        assert all(retrieved_vc[f"agent_{i}"] == 1 for i in range(20))

    def test_null_value(self):
        """None value is valid state."""
        coordinator = StateCoordinator()
        coordinator.set("conv", "key", None, "agent_1", {})
        assert coordinator.get("conv", "key") is None

    def test_complex_value_types(self):
        """Complex values (dict, list) are stored correctly."""
        coordinator = StateCoordinator()
        complex_value = {"nested": [1, 2, {"deep": "value"}]}
        coordinator.set("conv", "key", complex_value, "agent_1", {})
        retrieved = coordinator.get("conv", "key")
        assert retrieved == complex_value
        assert retrieved["nested"][2]["deep"] == "value"

    def test_timestamp_at_zero(self):
        """Timestamp value of 0.0 is valid."""
        coordinator = StateCoordinator()

        with patch("openclaw_gateway.state_coordinator.datetime") as mock_dt:
            mock_dt.now.return_value.timestamp.return_value = 0.0
            coordinator.set("conv", "key", "value1", "agent_1", {})

        # Should be stored
        assert coordinator.get("conv", "key") == "value1"

        # Another write at 1.0 should replace it
        with patch("openclaw_gateway.state_coordinator.datetime") as mock_dt:
            mock_dt.now.return_value.timestamp.return_value = 1.0
            coordinator.set("conv", "key", "value2", "agent_2", {})

        assert coordinator.get("conv", "key") == "value2"

    def test_vector_clock_with_zero_values(self):
        """Vector clock with explicit zero values is valid."""
        coordinator = StateCoordinator()
        vc = {"agent_1": 0, "agent_2": 0}
        coordinator.set("conv", "key", "val", "agent_1", vc)
        assert vc["agent_1"] == 1
...
class TestErrorHandling:
    ...
    def test_callback_with_different_exception_types(self):
        """Various exception types in callbacks don't crash."""
        coordinator = StateCoordinator()
        conversation_id = "conv_errors"

        def callback_value_error(key, value):
            raise ValueError("Invalid value")

        def callback_type_error(key, value):
            raise TypeError("Wrong type")

        def callback_key_error(key, value):
            raise KeyError("Missing key")

        good_callback = MagicMock()

        coordinator.subscribe(conversation_id, callback_value_error)
        coordinator.subscribe(conversation_id, callback_type_error)
        coordinator.subscribe(conversation_id, callback_key_error)
        coordinator.subscribe(conversation_id, good_callback)

        # Should not raise
        coordinator.set(conversation_id, "key", "value", "agent_1", {})

        # Good callback should still be called
        good_callback.assert_called_once_with("key", "value")

    def test_wal_recovery_with_missing_fields(self):
        """WAL recovery handles entries with missing optional fields gracefully."""

        async def test_recovery():
            coordinator = StateCoordinator()

            # Entry with missing vector_clock
            wal_entries = [
                {
                    "component": "state_coordinator",
                    ...
                    "data": {
                        ...
                        "value": "value_1",
                        "agent_id": "agent_1",
                        # vector_clock missing - should default to {}
                        "timestamp_utc": 100.0,
                    },
                }
            ]
    ...
    def test_wal_recovery_with_missing_timestamp(self):
        """WAL recovery handles missing timestamp with default."""
            ...
            wal_entries = [
                {
                    ...
                    "data": {
                        ...
                        "agent_id": "agent_1",
                        "vector_clock": {"agent_1": 1},
                        # timestamp_utc missing - should default to 0.0
                    },
                }
            ]

            await coordinator.recover_from_wal(wal_entries)
            assert coordinator.get("conv_ts", "key") == "value"

        asyncio.run(test_recovery())

    def test_subscription_after_state_set(self):
        """Subscribing after state is set doesn't fire for past updates."""
        coordinator = StateCoordinator()
        conversation_id = "conv_late"

        # Set initial state
        coordinator.set(conversation_id, "key1", "value1", "agent_1", {})

        # Subscribe after
        callback = MagicMock()
        coordinator.subscribe(conversation_id, callback)

        # Callback should not be called for past updates
        callback.assert_not_called()

        # But should be called for future updates
        coordinator.set(conversation_id, "key2", "value2", "agent_1", {})
        callback.assert_called_once_with("key2", "value2")

    def test_large_number_of_conversations(self):
        """System handles many concurrent conversations."""
        coordinator = StateCoordinator()

        # Create 1000 conversations with state
        for i in range(1000):
            coordinator.set(f"conv_{i}", "key", f"value_{i}", "agent_1", {})

        # Verify all are accessible
        for i in range(1000):
            assert coordinator.get(f"conv_{i}", "key") == f"value_{i}"

        # Vector clocks should be independent
        for i in range(1000):
            vc = coordinator.get_vector_clock(f"conv_{i}")
            assert vc["agent_1"] == 1

    def test_multiple_keys_per_conversation(self):
        """Single conversation can have many key-value pairs."""
        coordinator = StateCoordinator()
        conversation_id = "conv_multikey"
        vc = {"agent_1": 0}

        # Set 100 different keys with shared vector clock
        for i in range(100):
            coordinator.set(
                conversation_id, f"key_{i}", f"value_{i}", "agent_1", vc
            )

        # Verify all are accessible
        for i in range(100):
            assert coordinator.get(conversation_id, f"key_{i}") == f"value_{i}"

        # Vector clock should reflect all 100 writes
        assert vc["agent_1"] == 100

        # Aggregated VC should also show 100
        vc_agg = coordinator.get_vector_clock(conversation_id)
        assert vc_agg["agent_1"] == 100


class TestConcurrentMetadata:
    """Test vector clock correctness in complex concurrent scenarios."""

    def test_vector_clock_happened_before_detection(self):
        """_is_concurrent correctly detects non-concurrent clocks."""
        coordinator = StateCoordinator()

        vc1 = {"agent_1": 1, "agent_2": 0}
        vc2 = {"agent_1": 2, "agent_2": 0}

        # vc1 happened-before vc2 (vc1 <= vc2 element-wise)
        assert not coordinator._is_concurrent(vc1, vc2)
        assert not coordinator._is_concurrent(vc2, vc1)

    def test_vector_clock_concurrent_detection(self):
        """_is_concurrent correctly detects concurrent clocks."""
        coordinator = StateCoordinator()

        vc1 = {"agent_1": 1, "agent_2": 0}
        vc2 = {"agent_1": 0, "agent_2": 1}

        # Neither vc1 < vc2 nor vc2 < vc1: concurrent
        assert coordinator._is_concurrent(vc1, vc2)
        assert coordinator._is_concurrent(vc2, vc1)

    def test_vector_clock_equal_vectors(self):
        """Equal vector clocks are not concurrent."""
        coordinator = StateCoordinator()

        vc1 = {"agent_1": 1, "agent_2": 1}
        vc2 = {"agent_1": 1, "agent_2": 1}

        # Same vectors: not concurrent (vc1 = vc2 implies vc1 <= vc2)
        assert not coordinator._is_concurrent(vc1, vc2)

    def test_timestamp_fractional_precision(self):
        """Timestamps maintain fractional precision for ordering."""
        coordinator = StateCoordinator()

        with patch("openclaw_gateway.state_coordinator.datetime") as mock_dt:
            mock_dt.now.return_value.timestamp.return_value = 100.1
            coordinator.set("conv", "key", "value1", "agent_1", {})

        with patch("openclaw_gateway.state_coordinator.datetime") as mock_dt:
            mock_dt.now.return_value.timestamp.return_value = 100.2
            coordinator.set("conv", "key", "value2", "agent_2", {})

        # Even small fractional difference matters
        assert coordinator.get("conv", "key") == "value2"

        with patch("openclaw_gateway.state_coordinator.datetime") as mock_dt:
            mock_dt.now.return_value.timestamp.return_value = 100.15
            coordinator.set("conv", "key", "value3", "agent_3", {})

        # 100.15 < 100.2, so it loses
        assert coordinator.get("conv", "key") == "value2"


class TestStateEntryMetadata:
    """Test StateEntry data structure correctness."""

    def test_state_entry_vector_clock_copied(self):
        """StateEntry stores a copy of vector clock, not reference."""
        coordinator = StateCoordinator()
        vc = {"agent_1": 1}
        ...
        # Internal state should have preserved the value at time of set
        vc_retrieved = coordinator.get_vector_clock("conv")
        assert vc_retrieved["agent_1"] == 2  # incremented by set, not 999

    def test_state_entry_value_not_deep_copied(self):
        """StateEntry stores reference to mutable values (not deep copied)."""
        coordinator = StateCoordinator()
        mutable_value = {"data": [1, 2, 3]}

        coordinator.set("conv", "key", mutable_value, "agent_1", {})

        # Mutate the original
        mutable_value["data"].append(4)

        # Retrieved value reflects the mutation (same object)
        retrieved = coordinator.get("conv", "key")
        assert retrieved["data"] == [1, 2, 3, 4]
